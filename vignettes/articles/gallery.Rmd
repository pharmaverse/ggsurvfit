...
One of my favorite features of the {ggsurvfit} package is that any {ggplot2} to function may be used to modify the plot and the risk tables will still align with the primary plot.
This is accomplished by delaying the construction of the risk tables until the plot is printed.

For this reason, we must use the `ggsurvfit_build()` function when placing figures with risk tables side-by-side; this function will construct the risk tables and combine them with the primary plot.
...
For this reason, the `ggsurvfit()` and `ggcuminc()` functions do not modify the default ggplot scales; rather, all changes to the scales are left to the user.
...
To use this function, however, you must include all modifications you wish to make to either the x or y scale in this function call.
