...
Plots created with the {ggsurvfit} can be cobbled or patched together using the
grammar introduced in the {patchwork} package.

Importantly, you cannot use the `+` operator as you can with {patchwork}: use `|` instead.
There are other important caveats as well.
See `?ggsurvfit_arithmetic` for details.
...
NOTE: You can also use the patchwork arithmetic operators, but you must first wrap the plot in `patchwork::wrap_elements()`, e.g. `wrap_elements(built_p) | wrap_elements(built_p)`.
