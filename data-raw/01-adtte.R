...
attr(adtte$STR01L, "label") <- "Hormone receptor positive (Long format)"
...
attr(adtte$STR02L, "label") <- "Prior Radiotherapy at randomisation (Long format)"
...
attr(adtte$PARAM, "label") <- "Analysis parameter: Progression free survival"
attr(adtte$PARAMCD, "label") <- "Analysis parameter code"
attr(adtte$AVAL, "label") <- "Analysis value (time to event [days])"
